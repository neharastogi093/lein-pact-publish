...
(defn- publish-pacts-to-broker [pact-broker-url pact-directory version]
  (leiningen.core.main/info (str "INFO: Getting all pact files from " pact-directory))
    ...
    (if (.exists pact-file-dir)
      (let [error-count (atom 0)
        ...
        (doseq [pact-file filtered-pact-files]
          (leiningen.core.main/info (str "INFO: Uploading file: " (.getName pact-file)))
          (let [result (.uploadPactFile pact-broker-client pact-file version)]
            (leiningen.core.main/info (str "INFO: Uploading result '" result "' for file '" (.getName pact-file) "'"))
